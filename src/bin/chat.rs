...
use lmrs::script::write_script_to_tempfile;
...
const PIPER_SCRIPT: &str = include_str!("../piper.sh");
...
fn main() {
    ...
    let temp_script_path = write_script_to_tempfile(PIPER_SCRIPT, "piper.sh")
        .expect("Failed to write piper.sh to a temporary file");

    let mut child = Command::new(temp_script_path)
        .arg("say")
        .stdin(Stdio::piped())
        .spawn()
        .expect("Failed to start piper.sh");
