...
impl<'a> PHI3VProcessor<'a> {
    pub fn new(data: &'a [u8]) -> PHI3VProcessor<'a> {
        ...
        let img_projection0_quant = init_param_quant(
            data,
            &mut offset,
            1,
            text_dim * hidden_dim,
        ...
        let img_projection1_quant = init_param_quant(
            data,
            &mut offset,
            1,
            text_dim * text_dim,
            ...
            .for_each(|(h, xb)| {
                    ...
                    if !quantized {
                        ...
                        if p.q_type == QuantType::Q8_0 {
                            ...
                            matmul_q8(
                                &mut hidden_emb,
                                &sxq,
                                &w.img_projection0_quant.assume_init()[0],
                                hidden_dim as usize,
                ...
                for k in 0..n_simd {
                    let w1_bias_vec = f32x8::from(
                        &w.img_projection0_bias[(k * 8) as usize..(k * 8 + 8) as usize],
                    );
                ...
                unsafe {
                    if !quantized {
                        ...
                        let mut sxq = MutableQuantizedTensor {
                            q: &mut vec![0; (p.text_dim) as usize],
                            s: &mut vec![0.0; p.text_dim as usize],
                        };
                        ...
                        if p.q_type == QuantType::Q8_0 {
                            ...
                            matmul_q8(
                                xb,
                                &sxq,
                                &w.img_projection1_quant.assume_init()[0],
                                p.text_dim as usize,
                                p.group_size as usize,
                            );
                        } else if p.q_type == QuantType::Q4_0 {
                ...
                for k in 0..n_simd {
                    let w2_bias_vec = f32x8::from(
                        &w.img_projection1_bias[(k * 8) as usize..(k * 8 + 8) as usize],
                    );
    ...
    fn drop(&mut self) {
        if self.args.q_type != QuantType::None {
            unsafe {
                ...
                dealloc(
                    self.weights.img_projection0_quant.assume_init().as_ptr() as *mut u8,
                    weights_layout,
                );
                dealloc(
                    self.weights.img_projection1_quant.assume_init().as_ptr() as *mut u8,
                    weights_layout,
                );
            }
        }
