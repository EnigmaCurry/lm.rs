...
impl<'a> Transformer<'a> {
    pub fn new(data: &'a Mmap) -> (Transformer<'a>, usize) {
        ...
        if !quantized {
            ...
            if cfg.model_type == ModelType::PHI {
                lm_head =
                    MaybeUninit::new(init_param(data, &mut offset, 1, cfg.dim * cfg.vocab_size));
            }
        ...
        if cfg.model_type == ModelType::PHI {
            lm_head_quant = MaybeUninit::new(init_param_quant(
                data,
                &mut offset,
                1,
    ...
    fn forward_layer(&mut self, x: &mut [f32], l: u32, pos: u32) {
        ...
        s.xb3
            ...
            .for_each(|(h, xb)| {
                let q = &s.q
                    [(h as u32 * head_size) as usize..(h as u32 * head_size + head_size) as usize];
        ...
        if p.model_type == ModelType::GEMMA {
            ...
            unsafe {
                rmsnorm(
                    &mut s.xb,
                    x,
                    &w.w_rms_pre_ffn.assume_init()[(l * dim) as usize..(l * dim + dim) as usize],
                    dim as usize,
                    p.rms_norm_eps,
                    true,
                );
        ...
        for i in 0..hidden_dim {
            ...
            if p.model_type == ModelType::GEMMA {
                ...
                val *= 0.5
                    * (1.0
                        + ((0.7978845608028654 * (val + 0.044715 * val * val * val) as f64).tanh())
                            as f32);
            } else {
                // SiLU
                val *= 1.0 / (1.0 + (-val).exp());
        ...
        if p.model_type == ModelType::GEMMA {
            unsafe {
                rmsnorm(
                    &mut s.xb2,
                    &s.xb,
                    &w.w_rms_post_ffn.assume_init()[(l * dim) as usize..(l * dim + dim) as usize],
                    dim as usize,
                    p.rms_norm_eps,
                    true,
                );
    ...
    pub fn get_embeddings(&self, tokens: &[u32]) -> Vec<f32> {
        ...
        for t in tokens.iter().take(n_tokens) {
            out_embeddings.extend(
                &self.weights.token_embedding_table[(t * dim) as usize..(t * dim + dim) as usize],
            );
        }
